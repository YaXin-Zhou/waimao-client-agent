  ...
  return <div className="app-shell">
    ...
    <main className="main-shell">
      ...
      <div className="content">
        ...
        <div className="task-context"><label>当前获客任务<select value={remoteTaskId} onChange={selectTask} disabled={!remoteTasks.length}><option value="">暂无可选任务</option>{remoteTasks.map((task) => <option key={task.id} value={task.id}>{task.name}</option>)}</select></label>{remoteTaskConfig && <><span>任务条件：{remoteTaskConfig.criteria?.product || '未配置产品'} · {remoteTaskConfig.criteria?.countries?.join('、') || '未配置市场'} · 目标 {remoteTaskConfig.criteria?.qualified_lead_limit || 10} 家合格客户 · 业务 {remoteTaskConfig.criteria?.business_offerings?.length || 0} 项 · 背调字段 {remoteTaskConfig.criteria?.research_fields?.length || 0} 项</span>{discoveryRun?.status === 'running' && <small className="discovery-progress">搜索进度：{discoveryStepLabel(discoveryRun.step)} · 候选 {discoveryRun.candidate_count} · 官网 {discoveryRun.website_count} · 邮箱 {discoveryRun.public_email_count} · 合格 {discoveryRun.qualified_count}</small>}{discoveryRun?.status === 'failed' && <small className="discovery-progress discovery-failed">上次搜索失败：{discoveryRun.error}</small>}<button type="button" className="text-button task-edit-button" onClick={() => setShowRuleEditor(true)}>编辑研究规则</button><button type="button" className="outline-button task-discover-button" disabled={discoverLoading || discoveryRun?.status === 'running'} onClick={discoverLeads}>{discoverLoading || discoveryRun?.status === 'running' ? '搜索中…' : '开始搜索客户'}</button><button type="button" className="outline-button task-discover-button" onClick={() => setShowBrowserImport(true)}>导入浏览器结果</button></>}</div>
