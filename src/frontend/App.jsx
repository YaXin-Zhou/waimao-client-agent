    ...
    <main className="main-shell">
      ...
      <div className="content">
        ...
        <section className="workspace-grid">
          ...
          <aside className={`detail-panel panel ${filteredLeads.length ? '' : 'detail-empty'}`}>{filteredLeads.length ? <><div className="detail-top"><div className="company-symbol">◎</div><div className="company-title"><div><h2>{selected.name} <a href={selected.website} target="_blank" rel="noreferrer"><Icon name="external" size={14}/></a></h2><p>{selected.country} <i/> {selected.type} <i/> 成立信息待核验</p></div><div className="score-block"><Status status={selected.status}/><strong>{selected.score}<small> / 100</small></strong><span>匹配分数</span></div></div></div><div className="detail-tabs">{['概览', '来源证据', '开发信草稿'].map((tab) => <button className={detailTab === tab ? 'active' : ''} key={tab} onClick={() => setDetailTab(tab)}>{tab}</button>)}</div>{detailTab === '概览' && <Overview lead={selected} onEvidence={() => setDetailTab('来源证据')}/>} {detailTab === '来源证据' && <Evidence lead={selected}/>} {detailTab === '开发信草稿' && <Draft lead={selected} language={language} setLanguage={setLanguage} status={draftStatus} setStatus={setDraftStatus} notify={notify}/>}</> : <div className="detail-empty-state"><strong>没有选中的客户</strong><span>调整搜索词后选择一条客户记录</span></div>}</aside>
        </section>
      </div>
    </main>
...
function Draft({ lead, language, setLanguage, status, setStatus, notify }) { const body = language === '中文' ? `您好，${lead.name} 团队：\n\n我们是一家专注于便携式太阳能发电机及相关配件的中国制造商。了解到贵司在德国市场提供太阳能产品，我们想与您探讨产品目录是否有互补的机会。\n\n如果方向合适，我可以发送产品资料供您参考。` : `Hello ${lead.name} team,\n\nWe are a Chinese manufacturer focused on portable solar generators and related accessories. We noticed that your company offers solar products in Germany and would like to explore whether our product range could complement your current offering.\n\nIf relevant, I would be happy to share our product catalogue for your review.`; const recipient = lead.email && lead.email.includes('@') ? lead.email : '未发现公开邮箱'; const canApprove = recipient.includes('@'); return <div className="detail-body draft-view"><div className="section-title"><h3>开发信草稿</h3><div className="language-toggle">{['中文', 'EN'].map((item) => <button key={item} className={language === item ? 'active' : ''} onClick={() => setLanguage(item)}>{item}</button>)}</div></div><div className="review-banner"><span className="review-pulse"/><div><strong>{status === 'approved' ? '已批准' : status === 'revision' ? '需修改' : '待人工审核'}</strong><p>{status === 'approved' ? '审核完成，尚未发送' : '发送前必须由人工确认收件人与正文'}</p></div></div><div className="email-preview"><div className="email-meta"><span>收件人</span><strong>{recipient}</strong></div><div className="email-meta"><span>主题</span><strong>{language === '中文' ? '关于便携式太阳能产品合作' : 'Portable solar products for your range'}</strong></div><div className="email-copy">{body.split('\n').map((line, index) => <p key={`${line}-${index}`}>{line || '\u00a0'}</p>)}</div><div className="evidence-tag"><Icon name="clipboard" size={13}/>内容基于 2 条公开证据</div></div><div className="draft-actions"><button className="more-button" onClick={() => notify('更多操作将在后续阶段接入')}>···</button><button className="outline-button" onClick={() => { setStatus('revision'); notify('已退回修改，已记录审核意见') }}>退回修改</button><button className="primary-button" disabled={status === 'approved' || !canApprove} onClick={() => { if (canApprove) { setStatus('approved'); notify('草稿已批准，仍未发送邮件') } else notify('未发现公开邮箱，不能批准发送') }}><Icon name="check" size={16}/>{status === 'approved' ? '已批准' : '批准草稿'}</button></div></div> }
