...
function App() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/tasks/${remoteTaskId}/leads/${selected.domain}`)
      .then((response) => response.ok ? response.json() : Promise.reject(new Error('lead detail failed')))
      .then((data) => { if (!cancelled) setSelectedResearch(data.research || null) })
  ...
  const activeLead = selectedResearch ? { ...selected, type: customerTypeLabel(selectedResearch.customer_type), detail: selectedResearch.business_summary, research: selectedResearch, isRemote: true } : selected
...
function Draft({ lead, language, setLanguage, status, setStatus, notify }) { const isDraftAvailable = !lead.isRemote; const body = !isDraftAvailable ? '真实开发信尚未生成，当前不会发送任何邮件。' : language === '中文' ? `您好，${lead.name} 团队：\n\n我们是一家专注于便携式太阳能发电机及相关配件的中国制造商。了解到贵司在德国市场提供太阳能产品，我们想与您探讨产品目录是否有互补的机会。\n\n如果方向合适，我可以发送产品资料供您参考。` : `Hello ${lead.name} team,\n\nWe are a Chinese manufacturer focused on portable solar generators and related accessories. We noticed that your company offers solar products in Germany and would like to explore whether our product range could complement your current offering.\n\nIf relevant, I would be happy to share our product catalogue for your review.`; const recipient = lead.email && lead.email.includes('@') ? lead.email : '未发现公开邮箱'; const canApprove = isDraftAvailable && recipient.includes('@'); return <div className="detail-body draft-view"><div className="section-title"><h3>开发信草稿</h3>{isDraftAvailable && <div className="language-toggle">{['中文', 'EN'].map((item) => <button key={item} className={language === item ? 'active' : ''} onClick={() => setLanguage(item)}>{item}</button>)}</div>}</div><div className="review-banner"><span className="review-pulse"/><div><strong>{isDraftAvailable ? (status === 'approved' ? '已批准' : status === 'revision' ? '需修改' : '待人工审核') : '尚未生成'}</strong><p>{isDraftAvailable ? (status === 'approved' ? '审核完成，尚未发送' : '发送前必须由人工确认收件人与正文') : '真实 DeepSeek 开发信服务尚未接入'}</p></div></div><div className="email-preview"><div className="email-meta"><span>收件人</span><strong>{recipient}</strong></div><div className="email-meta"><span>主题</span><strong>{isDraftAvailable ? (language === '中文' ? '关于便携式太阳能产品合作' : 'Portable solar products for your range') : '尚未生成'}</strong></div><div className="email-copy">{body.split('\n').map((line, index) => <p key={`${line}-${index}`}>{line || '\u00a0'}</p>)}</div><div className="evidence-tag"><Icon name="clipboard" size={13}/>{isDraftAvailable ? '内容基于真实背调证据' : '当前没有真实开发信内容'}</div></div><div className="draft-actions"><button className="more-button" disabled={!isDraftAvailable} onClick={() => notify('更多操作将在后续阶段接入')}>···</button><button className="outline-button" disabled={!isDraftAvailable} onClick={() => { setStatus('revision'); notify('已退回修改，已记录审核意见') }}>退回修改</button><button className="primary-button" disabled={status === 'approved' || !canApprove} onClick={() => { if (canApprove) { setStatus('approved'); notify('草稿已批准，仍未发送邮件') } else notify(isDraftAvailable ? '未发现公开邮箱，不能批准发送' : '真实开发信尚未生成，不能批准发送') }}><Icon name="check" size={16}/>{status === 'approved' ? '已批准' : '批准草稿'}</button></div></div> }
