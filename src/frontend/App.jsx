  ...
  return <div className="app-shell">
    ...
    <main className="main-shell">
      ...
      <div className="content">
        ...
        <div className="task-context"><label>当前获客任务<select value={remoteTaskId} onChange={selectTask} disabled={!remoteTasks.length}><option value="">暂无可选任务</option>{remoteTasks.map((task) => <option key={task.id} value={task.id}>{task.name}</option>)}</select></label>{remoteTaskConfig && <><span>任务条件：{remoteTaskConfig.criteria?.product || '未配置产品'} · {remoteTaskConfig.criteria?.countries?.join('、') || '未配置市场'} · 目标 {remoteTaskConfig.criteria?.qualified_lead_limit || 10} 家合格客户 · 业务 {remoteTaskConfig.criteria?.business_offerings?.length || 0} 项 · 背调字段 {remoteTaskConfig.criteria?.research_fields?.length || 0} 项</span>{discoveryRun?.status === 'running' && <small className="discovery-progress">搜索进度：{discoveryStepLabel(discoveryRun.step)} · 候选 {discoveryRun.candidate_count} · 官网 {discoveryRun.website_count} · 邮箱 {discoveryRun.public_email_count} · 合格 {discoveryRun.qualified_count}</small>}{discoveryRun?.status === 'failed' && <small className="discovery-progress discovery-failed">{discoveryRun.user_message || '搜索未完成，请稍后重试。'}<details className="technical-details"><summary>技术详情</summary><span>{discoveryRun.error}</span></details></small>}<button type="button" className="outline-button task-discover-button" disabled={discoverLoading || discoveryRun?.status === 'running'} onClick={discoverLeads}>{discoverLoading || discoveryRun?.status === 'running' ? '搜索中…' : '开始搜索客户'}</button><details className={`maintenance-tools ${discoveryError?.code === 'search_provider_unavailable' ? 'has-warning' : ''}`} open={showMaintenanceTools} onToggle={(event) => setShowMaintenanceTools(event.currentTarget.open)}><summary>维护工具{discoveryError?.code === 'search_provider_unavailable' && <i aria-label="搜索源受限">!</i>}</summary><div className="maintenance-tools-body"><small>正常使用无需操作，仅供搜索源受限或需要调整任务配置时由技术人员处理。</small><button type="button" className="outline-button" onClick={() => { setShowMaintenanceTools(false); setShowRuleEditor(true) }}>编辑研究规则</button><button type="button" className="outline-button" onClick={() => { setShowMaintenanceTools(false); setShowBrowserImport(true) }}>备用：导入搜索结果</button></div></details></>}</div>
...
function DiscoveryFunnel({ summary }) {
  ...
  const rejectionSummary = Object.entries(summary.rejection_counts || {}).sort((a, b) => b[1] - a[1])
  const stages = [
...
function DiscoveryNotice({ error }) {
  ...
  return <div className="discovery-notice" role="status"><strong>{unavailable ? '搜索入口暂时不可用' : '搜索未完成'}</strong><span>{unavailable ? (error.user_message || '本次未生成客户记录，请稍后重试。') : error.message || '未生成客户记录，请检查搜索条件。'}</span>{error.retryable && <small>可稍后重试</small>}</div>
}
